...
from filmdemocracy.democracy.models import FilmDb, Film, Vote, Club
from filmdemocracy.utils.utils import user_is_club_member_check, user_is_club_admin_check
...
@login_required
def vote_film(request, club_id, film_public_id):
...
@login_required
def delete_vote(request, club_id, film_public_id):
    club = get_object_or_404(Club, id=club_id)
    if not user_is_club_member_check(request.user, club=club):
        return HttpResponseForbidden()
    film = get_object_or_404(Film, club=club, public_id=film_public_id)
...
@login_required
def delete_film(request, club_id, film_public_id):
    club = get_object_or_404(Club, id=club_id)
    if not user_is_club_member_check(request.user, club=club):
        return HttpResponseForbidden()
    film = get_object_or_404(Film, club=club, public_id=film_public_id)
...
@login_required
def unsee_film(request, club_id, film_public_id):
    club = get_object_or_404(Club, id=club_id)
